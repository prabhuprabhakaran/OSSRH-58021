...
/**
 * <p>A robust class for establishing a UDP server and manipulating its
 ...
 *
 * <p>To start a UDP server, create a new UniCastServer and call start():</p>
 *
 * <pre> UniCastServer server = new UniCastServer();
 * server.start();</pre>
 *
 ...
 *  server.addUdpServerListener( new UniCastServer.Adapter(){
 *     public void udpServerPacketReceived( UniCastServer.Event evt ){
 ...
 * an instance field to hold a UniCastServer where you'd have to pass along all
 * the setPort(...) methods and so forth.</p>
 *
 ...
 * You're welcome.</p>
 *
 * <p>Since the TcpServer.java, UniCastServer.java, and NioServer.java are so
 ...
 * UniCastServer.java file to your own package and use it as you like. Enjoy.
 * Please consider leaving the following statement here in this code:</p>
 *
 * <p><em>This <tt>UniCastServer</tt> class was copied to this project from its
...
public class UniCastServer {
    ...
    private Collection<UniCastServer.Listener> listeners = new LinkedList<UniCastServer.Listener>();    // Event listeners
    private UniCastServer.Event event = new UniCastServer.Event(this);                                  // Shared event
    private PropertyChangeSupport propSupport = new PropertyChangeSupport(this);                // Properties
    private final UniCastServer This = this;                                                // To aid in synchronizing
     ...
     */
    /**
     * Constructs a new UniCastServer that will listen on the default port 8000
     * (but not until {@link #start} is called). The I/O thread will not be in
     * daemon mode.
     */
    ...
    /**
     * Constructs a new UniCastServer that will listen on the given port (but
    ...
    public synchronized void start() {
        if (this.currentState == UniCastServer.State.STOPPED) { // Only if we're stopped now
            ...
            Runnable run = new Runnable() {
                ...
                public void run() {
                    ...
                    ioThread = null;
                    setState(UniCastServer.State.STOPPED); // Clear thread
                }   // end run
            };  // end runnable
            ...
            setState(UniCastServer.State.STARTING);        // Update state
    ...
    public synchronized void stop() {
        if (this.currentState == UniCastServer.State.STARTED) {         // Only if already STARTED
            setState(UniCastServer.State.STOPPING);                // Mark as STOPPING
     ...
     *
     * @param l the UniCastServer.Listener
     */
     ...
     * <code>UniCastServer.setLoggingLevel(Level.OFF);</code>.
     *
     ...
     */
    /**
     * An interface for listening to events from a {@link UniCastServer}. A
     ...
     * this UniCastServer.java file to your own package and use it as you like.
     * Enjoy. Please consider leaving the following statement here in this
     * code:</p>
     *
     * <p><em>This <tt>UniCastServer</tt> class was copied to this project from
     ...
     * {@link UniCastServer.Listener} interface with empty methods.
     *
     * <p>This code is released into the Public Domain. Since this is Public
     * Domain, you don't need to worry about licensing, and you can simply copy
     * this UniCastServer.java file to your own package and use it as you like.
     ...
     * @see Event
     */
//    public class Adapter implements UniCastServer.Listener {
    /**
     * Empty call for {@link UniCastServer.Listener#udpServerPacketReceived}.
     *
     * @param evt the event
     */
//        @Override
...
//    }   // end static inner class Adapter
     ...
     */
    /**
     * An event representing activity by a {@link UniCastServer}.
     *
     * <p>This code is released into the Public Domain. Since this is Public
     * Domain, you don't need to worry about licensing, and you can simply copy
     * this UniCastServer.java file to your own package and use it as you like.
    ...
    public static class Event extends java.util.EventObject {
        ...
        /**
         * Creates a Event based on the given {@link UniCastServer}.
         *
        ...
        /**
         * Returns the source of the event, a {@link UniCastServer}. Shorthand
         * for <tt>(UniCastServer)getSource()</tt>.
         *
         * @return the server
         */
         ...
         * {@link UniCastServer}. Shorthand for
