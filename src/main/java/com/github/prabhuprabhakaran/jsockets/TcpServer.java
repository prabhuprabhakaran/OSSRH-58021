...
/**
 * <p>A robust class for establishing a TCP server and manipulating its
 ...
 * <code>server.setExecutor(null)</code>. By default a cached thread pool is
 * used (
 ...
 * an instance field to hold a TcpServer where you'd have to pass along all the
 ...
 * TcpServer.java file to your own package and use it as you like. Enjoy. Please
 * consider leaving the following statement here in this code:</p>
 *
 * <p><em>This <tt>TcpServer</tt> class was copied to this project from its
...
public class TcpServer {
    ...
    /**
     * Constructs a new TcpServer that will listen on the default port 1234 (but
    ...
    /**
     * Constructs a new TcpServer that will listen on the given port (but not
    ...
    /**
     * Fires an event declaring the current state of the server. This may
...
//                        lSocketReader.close();
    ...
    public synchronized String getMessageAsString() {
        ...
        if (!getSocket().isClosed()) {
            try {
                InputStream in = getSocket().getInputStream();      // Input
                //OutputStream out = getSocket().getOutputStream();   // Output
                byte[] buff = new byte[getSocket().getReceiveBufferSize()];        // Buffer
                int num = -1;                                           // Bytes read
                while ((num = in.read(buff)) >= 0) {                    // Not EOS yet
                    lReturn = new String(buff, 0, num);          // Echo to console
...
//                    lReturn = new String(buff, 0, num);          // Echo to console
     ...
     * the same, events will be fired, but the server will not actually reset.
     *
     ...
     * TcpServer. When the server state changes, the three objects will be
    ...
    /* ********                                                          ******** */
    /**
     * An interface for listening to events from a {@link TcpServer}. A single
     ...
     * this TcpServer.java file to your own package and use it as you like.
...
//    }   // end static inner class Adapter
    ...
    public static class Event extends java.util.EventObject {
        ...
        /**
         * Returns the source of the event, a {@link TcpServer}. Shorthand for
         * <tt>(TcpServer)getSource()</tt>.
         *
         * @return the server
         */
         ...
         * {@link TcpServer}. Shorthand for <tt>getTcpServer().getPacket()</tt>.
         *
