 ...
 *
 * <p>
 * To start a UDP server, create a new UniCastServer and call start():</p>
 *
 ...
 * still have the power to introduce these problems yourself.</p>
 *
 * <p>
 * It's often handy to have your own class extend this one rather than making an
 * instance field to hold a UniCastServer where you'd have to pass along all the
 ...
 *
 * <p>
 * Since the TcpServer.java, UniCastServer.java, and NioServer.java are so
 ...
 *
 * <p>
 * <em>This <tt>UniCastServer</tt> class was copied to this project from its
...
public class UniCastClient {
    ...
    /**
     * <p>
     * One of four possible states for the server to be in:</p>
     *
    ...
    private Collection<UniCastClient.Listener> listeners = new LinkedList<UniCastClient.Listener>();    // Event listeners
    private UniCastClient.Event event = new UniCastClient.Event(this);                                  // Shared event
    private PropertyChangeSupport propSupport = new PropertyChangeSupport(this);                // Properties
    private final UniCastClient This = this;                                                // To aid in synchronizing
    ...
    public synchronized void start() {
        if (this.currentState == UniCastClient.State.STOPPED) { // Only if we're stopped now
            ...
            Runnable run = new Runnable() {
                ...
                public void run() {
                    ...
                    ioThread = null;
                    setState(UniCastClient.State.STOPPED); // Clear thread
                }   // end run
            };  // end runnable
            ...
            setState(UniCastClient.State.STARTING);        // Update state
    ...
    public synchronized void stop() {
        if (this.currentState == UniCastClient.State.STARTED) {         // Only if already STARTED
            setState(UniCastClient.State.STOPPING);                // Mark as STOPPING
     ...
     */
    public synchronized void addUdpServerListener(UniCastClient.Listener l) {
        listeners.add(l);
    }
     ...
     * @param l the UniCastServer.Listener
     */
    public synchronized void removeUdpServerListener(UniCastClient.Listener l) {
        listeners.remove(l);
    }
...
//    }   // end static inner class Adapter
 ...
 /*
    ...
    public static class Event extends java.util.EventObject {
         ...
         * @return the server
         */
        public UniCastClient getUdpServer() {
