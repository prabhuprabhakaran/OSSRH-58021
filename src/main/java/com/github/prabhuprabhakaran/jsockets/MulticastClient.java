...
public class MulticastClient {
    ...
    private Collection<MulticastClient.Listener> listeners = new LinkedList<MulticastClient.Listener>();    // Event listeners
    private MulticastClient.Event event = new MulticastClient.Event(this);                                  // Shared event
    private PropertyChangeSupport propSupport = new PropertyChangeSupport(this);                // Properties
    private final MulticastClient This = this;                                                // To aid in synchronizing
    ...
    public synchronized void start() {
        if (this.currentState == MulticastClient.State.STOPPED) { // Only if we're stopped now
            ...
            Runnable run = new Runnable() {
                ...
                public void run() {
                    ...
                    ioThread = null;
                    setState(MulticastClient.State.STOPPED); // Clear thread
                }   // end run
            };  // end runnable
            ...
            setState(MulticastClient.State.STARTING);        // Update state
    ...
    public synchronized void stop() {
        if (this.currentState == MulticastClient.State.STARTED) {         // Only if already STARTED
            setState(MulticastClient.State.STOPPING);                // Mark as STOPPING
