...
/**
 * <p>A robust class for establishing a Udp Client and manipulating its
 * listening port and optionally a multicast groups to join. The {@link Event}s
 * and property change events make it an appropriate tool in a threaded, GUI
 ...
 *
 * <p>To start a Udp Client, create a new MulticastClient and call start():</p>
 *
 * <pre> MulticastClient server = new MulticastClient();
 * server.start();</pre>
 *
 * <p>Of course it won't be much help unless you know which port it's listening
 * on and you register as a listener so you'll know when a
 * <tt>java.net.DatagramPacket</tt> has come in:</p>
 *
 * <pre> server.setPort(1234);
 *  server.addUdpClientListener( new MulticastClient.Adapter(){
 *     public void UdpClientPacketReceived( MulticastClient.Event evt ){
 *         DatagramPacket packet = evt.getPacket();
 *         ...
 *     }   // end packet received
 * });</pre>
 *
 * <p>The server runs on one thread, and all events are fired on that thread. If
 ...
 * make a copy of the byte array.</p>
 *
 * <p>The full 64KB allowed by the UDP standard is set aside to receive the
 * datagrams, but it's possible that your host platform may truncate that.</p>
 *
 ...
 * an instance field to hold a MulticastClient where you'd have to pass along
 * all the setPort(...) methods and so forth.</p>
 *
 ...
 * You're welcome.</p>
 *
 * <p>Since the TcpServer.java, MulticastClient.java, and NioServer.java are so
 ...
 * MulticastClient.java file to your own package and use it as you like. Enjoy.
 * Please consider leaving the following statement here in this code:</p>
 *
 * <p><em>This <tt>MulticastClient</tt> class was copied to this project from
 * its source as found at <a href="http://iharder.net"
...
public class MulticastServer {
    ...
    private final static Logger LOGGER = Logger.getLogger(MulticastServer.class.getName());
    /**
    ...
    private Collection<MulticastServer.Listener> listeners = new LinkedList<MulticastServer.Listener>();    // Event listeners
    private MulticastServer.Event event = new MulticastServer.Event(this);                                  // Shared event
    private PropertyChangeSupport propSupport = new PropertyChangeSupport(this);                // Properties
    private final MulticastServer This = this;                                                // To aid in synchronizing
     ...
     */
    /**
     * Constructs a new MulticastClient that will listen on the default port
     * 8000 (but not until {@link #start} is called). The I/O thread will not be
     * in daemon mode.
     */
    ...
    /**
     * Constructs a new MulticastClient that will listen on the given port (but
     * not until {@link #start} is called). The I/O thread will not be in daemon
     * mode.
     *
     * @param port The initial port on which to listen
     */
    public MulticastServer(String host, int port) {
        this.host = host;
        this.port = port;
        setGroups(host);
    }
    ...
    /**
     * Constructs a new MulticastClient that will listen on the given port (but
     * not until {@link #start} is called). The provided ThreadFactory will be
     * used when starting and running the server.
     *
     * @param port The initial port on which to listen
     * @param factory The thread factory used to generate a thread to run the
     * server
     */
    public MulticastServer(String host, int port, ThreadFactory factory) {
        this.host = host;
        this.port = port;
        setGroups(host);
    ...
    public synchronized void start() {
        if (this.currentState == MulticastServer.State.STOPPED) { // Only if we're stopped now
            ...
            Runnable run = new Runnable() {
                ...
                public void run() {
                    ...
                    ioThread = null;
                    setState(MulticastServer.State.STOPPED); // Clear thread
                }   // end run
            };  // end runnable
            ...
            setState(MulticastServer.State.STARTING);        // Update state
    ...
    public synchronized void stop() {
        if (this.currentState == MulticastServer.State.STARTED) {         // Only if already STARTED
            setState(MulticastServer.State.STOPPING);                // Mark as STOPPING
     ...
     */
    public synchronized MulticastServer.State getState() {
     ...
     */
    protected synchronized void setState(MulticastServer.State state) {
                ...
                this.addPropertyChangeListener(STATE_PROP, new PropertyChangeListener() {
                    public void propertyChange(PropertyChangeEvent evt) {
                        ...
                        if (newState == State.STOPPED) {
                            MulticastServer server = (MulticastServer) evt.getSource();
     ...
     * @throws java.io.IOException if the server throws an exception or if the
     * server is not running (in which case there is no underlying server socket
    ...
    /**
     * <p>Sets the new multicast groups to which the server will join. If the
     * server is already listening, then it will attempt to restart, generating
     * start and stop events. </p>
     *
     * <p>The list of groups may be whitespace- and/or comma-separated. When the
     * server starts up (or restarts), the list will be parsed, and only
     * legitimate groups will actually be joined.</p> May be null.
     *
     ...
     *
     * @param l the MulticastClient.Listener
     */
    public synchronized void addUdpClientListener(MulticastServer.Listener l) {
        listeners.add(l);
    }
     ...
     * @param l the MulticastClient.Listener
     */
    public synchronized void removeUdpClientListener(MulticastServer.Listener l) {
        listeners.remove(l);
    }
    ...
    protected synchronized void fireUdpClientPacketReceived() {
        ...
        MulticastServer.Listener[] ll = listeners.toArray(new MulticastServer.Listener[listeners.size()]);
        for (MulticastServer.Listener l : ll) {
            try {
    ...
    }  // end fireUdpClientPacketReceived
     ...
     * <code>MulticastClient.setLoggingLevel(Level.OFF);</code>.
     *
     ...
     */
    /**
     * An interface for listening to events from a {@link MulticastClient}. A
     * single {@link Event} is shared for all invocations of these methods.
     *
     ...
     * this MulticastClient.java file to your own package and use it as you
     * like. Enjoy. Please consider leaving the following statement here in this
     * code:</p>
     *
     * <p><em>This <tt>MulticastClient</tt> class was copied to this project
     * from its source as found at <a href="http://iharder.net"
     * target="_blank">iHarder.net</a>.</em></p>
     *
     * @author Robert Harder
     * @author rharder@users.sourceforge.net
    ...
    public static interface Listener extends java.util.EventListener {
         ...
         * @see Event#getPacket
         */
        public abstract void packetReceived(MulticastServer.Event evt);
     ...
     * {@link MulticastClient.Listener} interface with empty methods.
     *
     * <p>This code is released into the Public Domain. Since this is Public
     * Domain, you don't need to worry about licensing, and you can simply copy
     * this MulticastClient.java file to your own package and use it as you
     ...
     * @see Event
     */
//    public class Adapter implements MulticastClient.Listener {
    /**
     * Empty call for {@link MulticastClient.Listener#UdpClientPacketReceived}.
     *
     * @param evt the event
     */
//        @Override
...
//    }   // end static inner class Adapter
     ...
     */
    /**
     * An event representing activity by a {@link MulticastClient}.
     *
     * <p>This code is released into the Public Domain. Since this is Public
     * Domain, you don't need to worry about licensing, and you can simply copy
     * this MulticastClient.java file to your own package and use it as you
    ...
    public static class Event extends java.util.EventObject {
        ...
        /**
         * Creates a Event based on the given {@link MulticastClient}.
         *
         * @param src the source of the event
         */
        public Event(MulticastServer src) {
            super(src);
        }

        /**
         * Returns the source of the event, a {@link MulticastClient}. Shorthand
         * for <tt>(MulticastClient)getSource()</tt>.
         *
         * @return the server
         */
        public MulticastServer getUdpClient() {
            return (MulticastServer) getSource();
        }

        /**
         * Shorthand for <tt>getUdpClient().getState()</tt>.
         *
         * @return the state of the server
         * @see MulticastClient.State
         */
        public MulticastServer.State getState() {
            return getUdpClient().getState();
        }

        /**
         * Returns the most recent datagram packet received by the
         * {@link MulticastClient}. Shorthand for
         * <tt>getUdpClient().getPacket()</tt>.
         *
         ...
         * to
         * <code>evt.getUdpClient.send( packet )</code>.
         *
         * @param packet the packet to send
         * @throws java.io.IOException if the server throws an exception or if
         * the server is not running (in which case there is no underlying
         * server socket to send the datagram)
         */
